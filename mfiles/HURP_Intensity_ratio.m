function HURP_Intensity_ratio(DATA)
%HURP_Intensity_ratio Summary of this function goes here
...
hfig = figure;
hfig.Name = 'HURP intensity ratio';
...
for idxC = 1:size(LBL_CONDITION,1)
    ...
    hr{idxC} = HURP_spindle_ratio;
    
    
    line(idxC+0.2*(rand(size(HURP_spindle_ratio))-0.5),HURP_spindle_ratio,...
        'LineStyle','none','Marker','o',...
        'MarkerEdgeColor',[0.2 0.2 0.2],'MarkerFaceColor',CMAP_CONDITION(idxC,:),...
        'Parent',ax)
    line(idxC+[-0.5*0.1 0.5*0.1],median(HURP_spindle_ratio)*ones(2,1),...
        'Color',[0 0 0],'LineWidth',2,'Parent',ax)
    line(idxC*ones(2,1),[prctile(HURP_spindle_ratio,25) prctile(HURP_spindle_ratio,75)],...
        'Color',[0 0 0],'LineWidth',1,'Parent',ax)
    line(idxC+[-0.5*0.05 0.5*0.05],prctile(HURP_spindle_ratio,25)*ones(2,1),...
        'Color',[0 0 0],'LineWidth',2,'Parent',ax)
    line(idxC+[-0.5*0.05 0.5*0.05],prctile(HURP_spindle_ratio,75)*ones(2,1),...
        'Color',[0 0 0],'LineWidth',2,'Parent',ax)     
  
end
title(ax,'HURP half-spindle ratios, Q1 median Q3')
line(get(ax,'XLIM'),[1 1],'Color',[0.2 0.2 0.2],'LineStyle',':','Parent',ax)
set(ax,'XTickLabel',txt_xlbl)
ylabel('HURP half-spindle ratio (-)')
...
elseif  size(LBL_CONDITION,1) == 4
    ...
    [p,tbl,stats,terms]  = anovan(LineProf.HURP_spindle(:,2)./LineProf.HURP_spindle(:,1),...
        {LineProf.Factor1,LineProf.Factor2},'model','interaction','varnames',LBL_FACTOR);        
    ht = findobj(gcf,'Type','Uicontrol','-and','Tag','Heading');
    ht.String = 'Analysis of HURP half-spindle ratio Variance';
    figure
    [c,m,h,gnames] = multcompare(stats,'Dimension',[1 2]);
    set(gcf,'Name',['HURP half-spindle ratio 2-way post-hoc ' get(gcf,'Name')])
    writetable(table([gnames(c(:,1)) gnames(c(:,2)) num2cell(c(:,end))]),[filepath filesep  'HURP ratio half-spindle.xlsx'])
